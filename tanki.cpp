...
public:
    int x, y;//ночалная координата построения припятсвий
...
int FindBrick(int x, int y)//поиск индекса в массиве препятсвий по координатам
{
    for (int i = 0; i < brickCnt; i++)
    {
        if (brick[i].x == x && brick[i].y == y)//если координаты совподают с координатой какогото из блоков массиве
        {
            return i;//то возвращаем индекс препятсвия
        }
    }
    return -1;//иначе возвращаем -1 что означает отсустаие блока с такими координатами в массиве
}

int status = 0;//переменная передающая состояние танка
int route = 0;//направление танка
int notstop = 0;//счетчик ступора
//реализация движения танка-бота
void Ttank::MoveBot()
{
    if (this->sX == this->x && this->sY == this->y)//когда танк убит, либо когда карта обновилась, танк возвращается в нулевое состояни
        status = 0;//нулевое состояние указывает путь боту на свою базу
    bool fire = false;//для того чтобы танк выстреливал 1 раз за итерацию

    //Выравниваем танк по оси y по отношению к базам
    if (status == 0)//нулевое состояние конечного автомата
    {
        if (this->y > base1Y)//Выравниваем координату бота по оси y по отношению к базам
        {
            route = 0;// направление движения вверх
        }
        else if (this->y < base1Y)//Выравниваем координату бота по оси y по отношению к базам
        {
            route = 1;//направление движения вниз
        }
        else if (this->y == base1Y)//Выравниваем координату бота по оси y по отношению к базам
        {
            route = 2;//направление движения бота влево
            status = 1;//статус разрушения по которому бот разрушает, препятсвия, противника на пути к базе, либо вражескую базу.
        }
    }
    int index = -1;//переменная которая хранит индщекс препятсвия на который смотрит бот
    //Ведем танк по заданному направлению, попутно уничтожая ttBrick
    if (status == 1)//это состояние при котором бот начинает разрушать препятсвия либо вражескую базу
    {

        if (route == 0)//при направлении движения бота вверх
            index = FindBrick(this->x, this->y + 3);//ищем блок который вверху бота и ищем индекс в массиве препятсвий
        if (route == 1)//при направлении движения бота вниз
            index = FindBrick(this->x, this->y - 3);//ищем блок который внизу бота и ищем индекс в массиве препятсвий и так далее
        if (route == 2)//лево
            index = FindBrick(this->x - 3, this->y);
        if (route == 3)//право
            index = FindBrick(this->x + 3, this->y);

        if ((brick[index].tp == ttBrick && brick[index].use == 1)//проверяем тип препятсвие на возможность разрушения
            || index == nbase1)
        {
            fire = true;//если возможно, то разрушает
        }
        if ((brick[index].tp == ttStone && brick[index].use == 1)//если это камень и он активный, либо эта база бота
            || brick[index].tp == ttbase2)
        {
            status = 2; // то ищем путь обхода в соостоянии 2
        }


    }
    //объезд не разрушаемых препятсвий
    if (status == 2)//при переходе в статус 2
    {
        int sroute = route;//бот запоминает текущее направление в котором он уперся на камень либо свою базу либо стены
        do
        {
            route = rand() % 4;//генерация направление 0-3
        } while (route == sroute);//генерация повторится до тех пор пока не появится новое направление
        status = 1;//бот переходит в статус в котором он разрушает объект
    }
        ...
        (CheckCrossAnyBrick(GetRect()) != 0))//или столкнулись с препятсвием
    {
        ...
        *this = old;//то мы не перемещаемся
        if (
            !((brick[index].tp == ttStone && brick[index].use == 1) &&//если препятвие активный камень
                (brick[index].tp == ttStone && brick[index].use == 0) &&//и если препятсвий неактивный камень
                (brick[index].tp == ttBrick && brick[index].use == 0))//и если препятсвие неактивный блок
            && index != nbase2)//и если это не база бота
            fire = true;//то бот понимает что это либо вражеский танк либо активный разрушаемый объект
        notstop++;//когда бот понял что перед ним вражеский танк он стреляет до тех пор пока его не убьет, либо пока танк не пропустит бота
        if (notstop == 5)//если бот уперается и не хочет разворачиваться более 5 итераций подрят
        {
            status = 2;//то он переходит в состояние 2 котрое его развернет
            notstop = 0;//обнуляем счетчик ступора
        }
    }
...
void CreateBattleField()
{
    ...
    for (int i = 5; i < width - 5; i += 3)
        for (int j = 1; j < height - 1; j += 3)//проходим по всей карте с отступом 5 клеток от краев, слева и справа, шаг делаем 3 клетки так как размер препятсвия 3*3
        {
            ...
            brick[pos].x = i;//начальная координата построения препятсвий
            brick[pos].y = j;//начальная координата построения препятсвий
            ...
            if (i == base1X && j == base2Y)
            {
                ...
                brick[pos].use = 1;
                brick[pos].SetPos(i, j);//отоброжение pos-того элемента как база1
            }
            else if (i == base2X && j == base2Y)
            {
                ...
                brick[pos].use = 1;
                brick[pos].SetPos(i, j);//отоброжение pos-того элемента как база2
            }
            else
            {
...
int main()
{
    ...
    while (true)
    {
        ...
        do
        {
            ...
            tank[1].MoveBot();//движение и стрельба танка-бот
        ...
        cout << endl;
        cout << RED << "\t\t\t\t\t\t\tResults" << RESET << endl;
        cout << BLUE << "\t\t\t\t\t\t    Player1 score: " << RESET << wins1 << endl;
        cout << GREEN <<"\t\t\t\t\t\t    Robot score\t : "<<RESET << wins2 << endl << endl;
        cout << endl;
    }
